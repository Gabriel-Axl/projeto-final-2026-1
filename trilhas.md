...
- **Produto:** API + painel que recebe o perfil do cliente, devolve a probabilidade de churn e os fatores que mais pesaram.
...
- **Produto:** endpoint que escora uma transação em tempo real como fraude/não-fraude, com o limiar de decisão exposto e justificado.
...
- **Produto:** API que estima o prazo ou o risco de atraso de um pedido.
...
- **Produto:** API de apoio à decisão que devolve a probabilidade, acompanhada de uma análise de equidade entre grupos (ex.: gênero, região) no relatório.
...
- **Produto:** widget/API que classifica um texto colado. Ecoa o PoC Consulta Ouvidoria.
...
- **Produto:** moderador que sinaliza comentários, com explicação do nível de ofensa.
...
- **Produto:** app "você pode gostar de…", que recebe um usuário (ou algumas notas) e devolve recomendações. A versão brasileira é recomendar produtos sobre o Olist.
...
**Produto:** um sistema de ML acessível e demonstrável ao vivo (API, bot, frontend ou dashboard). O tipo é livre; o que não muda é que precisa estar no ar até o Demo Day.
